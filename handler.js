...
export async function handler(chatUpdate) {
    ...
    global.img = 'https://telegra.ph/file/2d9a745fd579043994593.jpg' 
...
export async function participantsUpdate({ id, participants, action }) {
 ...
 /* conn.sendButtonDoc(id, wm, text, action == 'add' ? 'ᴡᴇʟᴄᴏᴍᴇ' : 'sᴀʏᴏɴᴀʀᴀᴀ', action === 'add' ? '.intro' : 'Rɪᴇʟʟ X Wᴀʜʏᴜ', fkontak, { contextInfo: { externalAdReply: { showAdAttribution: true,
  ...
  conn.sendButtonImg(id, await(await fetch(action === 'add' ? wel : lea)).buffer(), 'Group Messege', text, action == 'add' ? 'ᴡᴇʟᴄᴏᴍᴇ' : 'sᴀʏᴏɴᴀʀᴀᴀ', action === 'add' ? '.intro' : 'Rɪᴇʟʟ X Wᴀʜʏᴜ', fkontak, { contextInfo: { externalAdReply: { showAdAttribution: true,
